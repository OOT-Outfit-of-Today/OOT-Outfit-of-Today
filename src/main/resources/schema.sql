...
CREATE TABLE IF NOT EXISTS `users` (
    ...
    `trade_location` POINT SRID 4326,  -- 수정: SRID 4326 추가(WGS84 지리적 좌표계, GPS 표준)
...
CREATE TABLE IF NOT EXISTS `sale_posts` (
    ...
    `trade_address` VARCHAR(50) NOT NULL,
    `trade_location` POINT NOT NULL SRID 4326,  -- 수정: SRID 4326 추가(WGS84 지리적 좌표계, 경도/위도 범위 검증 및 지리적 연산 지원)
    `created_at` DATETIME(6) NOT NULL,
    `updated_at` DATETIME(6) NOT NULL,
    `deleted_at` DATETIME(6),
    `is_deleted` BIT(1) DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `sale_post_images` (
                                                  ...
                                                  `image_id` BIGINT NOT NULL,  -- 수정: image_url -> image_id로 변경(Image 엔티티 참조)
                                                  `display_order` INT NOT NULL,
                                                  `is_main` BIT(1) NOT NULL DEFAULT 0,  -- 추가: 대표 이미지 표시
    `created_at` DATETIME(6) NOT NULL,
    `updated_at` DATETIME(6) NOT NULL,
    `deleted_at` DATETIME(6),
    `is_deleted` BIT(1) DEFAULT 0,
    FOREIGN KEY (`sale_post_id`) REFERENCES sale_posts(`id`),
    FOREIGN KEY (`image_id`) REFERENCES images(`id`)  -- 추가: images 테이블 FK 제약
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `donation_centers` (
    ...
    `location` POINT NOT NULL SRID 4326,  -- SRID 4326: WGS84 지리적 좌표계
    `description` VARCHAR(255),
    `created_at` DATETIME(6) NOT NULL,
    `updated_at` DATETIME(6) NOT NULL,
    `deleted_at` DATETIME(6),
...
CREATE TABLE IF NOT EXISTS `recommendation_batch_history` (
    ...
    `success_users` INT NOT NULL,
    `failed_users` INT NOT NULL,
